# CLAUDE.md - MadGrav Development Guide

## Project Overview

MadGrav is a personal fork of MeerK40t (pronounced "MeerKat"), an open-source laser cutting/engraving control software. It provides a highly extensible, plugin-based platform supporting multiple laser hardware types including K40 (Lihuiyu), GRBL, Ruida, Moshiboard, Newly, and galvo (Balor) lasers.
...
pip install madgrav[all]

# Run application
python -m madgrav              # Full GUI
python -m madgrav --no-gui     # Console mode
python -m madgrav --simpleui   # Simplified interface
...
1. **Kernel** (`madgrav/kernel/`) - Service bus, plugin system, signals, channels, settings, jobs
2. **Core** (`madgrav/core/`) - Element/node tree, operations, cutcode, planning, spooling, units
3. **Device** (`madgrav/device/`) - Hardware abstraction layer and base device
4. **GUI** (`madgrav/gui/`) - wxPython interface with AUI docking
5. **Drivers** - Hardware-specific: `grbl/`, `lihuiyu/`, `ruida/`, `moshi/`, `newly/`, `balormk/`
6. **Extra** (`madgrav/extra/`) - File format parsers, tracing, fonts, parametric shapes, utilities
...
```python
from madgrav.kernel import signal_listener
...
```python
from madgrav.core.treeop import tree_operation, tree_conditional
...
Discovered automatically via the `madgrav.extension` setuptools entry-point group. Each must expose a `plugin(kernel, lifecycle)` callable. Disabled with `--no-plugins` or if `lifecycle == "invalidate"` returns True.
...
1. Create `madgrav/newdevice/` directory.
