"""
Internal plugins are a list of plugins which are included internally with MadGrav, usually additional features
are written like regular plugins and obey the rules for the madgrav/kernel plugin system. Plugins can contain
...
Provides all the core plugins for madgrav to run which are sub-references from that plugin file.
"""
...
def plugin(kernel, lifecycle):
    if lifecycle == "plugins":
        ...
        from madgrav.camera.plugin import plugin as camera_plugin
        ...
        from .qt.plugin import plugin as qtplugin

        plugins.append(qtplugin)
        ...
        from .extra.custom_laser_tools import plugin as customtoolsplugin

        plugins.append(customtoolsplugin)
