...
class Kernel(Settings):
    ...
    def open_safe(self, filename, *args, **kwargs):
        """
        ...
        try:
            ...
            if file_obj is not None and not file_obj.closed:
                ...
                return file_obj
            file_obj = open(filename, *args, **kwargs)
            ...
            return self.open_safe(filename, *args, **kwargs)
    ...
    def _start_debugging(self) -> None:
        ...
        debug_file = self.open_safe(filename, "a", encoding="utf-8")
        debug_file.write("\n\n\n")
            ...
            if klp(k) < LIFECYCLE_KERNEL_PREMAIN <= end:
                ...
                if hasattr(k, "premain"):
                    ...
                    if self._shutdown:
                        ...
                        # Skip straight past mainloop/postmain (start=POSTMAIN, not
                        # PRESHUTDOWN itself) so the "if start < PRESHUTDOWN <= end"
                        # check below still passes and plugins get their preshutdown
                        # hook -- setting start = PRESHUTDOWN made that check
                        # start < PRESHUTDOWN false, silently skipping every
                        # plugin's preshutdown lifecycle (object-level preshutdown
                        # handling was unaffected since it gates on each object's
                        # own klp(k), not this shared start/end pair).
                        # print (f"Shutdown requested during premain of {str(k)}: start was {KERNEL_LIFECYCLE_NAMES[start]}, end={KERNEL_LIFECYCLE_NAMES[end]}.")
                        start = LIFECYCLE_KERNEL_POSTMAIN
    ...
    def command_boot(self) -> None:
        ...
        def channel_save(channel, _, channel_name, filename=None, **kwargs):
            """
            Save a particular channel to disk. Any data sent to that channel within Madgrav will write out a log.
            """
            if channel_name is None:
                raise CommandSyntaxError(_("channel_name is not specified."))
            try:
                ...
                pass
            if filename is None:
                filename = f"MadGrav-channel-{datetime.now():%Y-%m-%d_%H_%M_%S}.txt"
        ...
        def setting_export(channel, _, remainder=None, **kwargs):
            ...
            newfile = os.path.join(
                exportdir, f"madgrav_{datetime.now():%Y-%m-%d_%H_%M_%S}.cfg"
            )
        ...
        @self.console_command(
            "restart", help=_("shuts down all processes, exits and restarts madgrav")
        )
        def restart(**kwargs):
            self.restart = True
            if self._shutdown:
