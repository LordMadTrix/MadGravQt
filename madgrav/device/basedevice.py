        ...
        def device_start(channel, _, name, data, label=None, **kwargs):
            ...
            try:
                ...
                if label:
                    ...
                    # choices is a list of {"attr": ..., "default": ...} dicts
                    # (see e.g. GRBLDevice.__init__, which does
                    # `for c in choices: setattr(self, c["attr"], c["default"])`)
                    # -- dict(choices) on a list of dicts doesn't raise, but
                    # silently produces garbage: it iterates each dict's KEYS
                    # (2 for every entry here) and treats them as a (key,
                    # value) pair, collapsing the whole list down to a single
                    # {"attr": "default"} entry. Copy each dict individually
                    # instead, both to fix that and to avoid mutating the
                    # dev_info registry's shared original (the same list
                    # object is reused for every future device created from
                    # this profile).
