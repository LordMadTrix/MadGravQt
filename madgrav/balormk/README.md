...
BalorMK began life as the madgrav plugin-driver for Balor by Bryce Schroeder which controls the LMC ezcad2 galvo laser boards. This was eventually rewritten but still uses many of the original names. Even as the backend was rewritten.

The BalorMK module provides comprehensive support for JCZ Technologies galvo laser controllers, including the EZCAD2 series and compatible devices. It implements the complete MadGrav device driver pattern with device service, driver, and controller components for high-performance galvo laser control.
...
- **`device.py`** - Main BalorDevice service class (1136 lines)
  - Inherits from MadGrav Service and Status classes
...
- **`driver.py`** - BalorDriver implementation (795 lines)
  - Translates MadGrav cutcode operations to galvo commands
...
- **`plugin.py`** - Device registration and plugin lifecycle
  - Registers BalorDevice with MadGrav kernel
...
- **MadGrav Kernel**: Full service integration with kernel channels and settings
- **Cutcode Processing**: Compatible with MadGrav's cutcode data structures
