...
from madgrav.device.gui.defaultactions import DefaultActionPanel
from madgrav.device.gui.effectspanel import EffectsPanel
from madgrav.device.gui.formatterpanel import FormatterPanel
from madgrav.device.gui.warningpanel import WarningPanel
from madgrav.gui.choicepropertypanel import ChoicePropertyPanel
from madgrav.gui.icons import icons8_administrative_tools
...
class NewlyConfiguration(MWindow):
    ...
    **End-User Perspective:**
    This panel provides user interface controls for newlyconfiguration functionality in MadGrav."""
        ...
        for item in options:
            ...
            if addpanel:
                newpanel = ChoicePropertyPanel(
                    self.notebook_main, wx.ID_ANY, context=self.context, choices=section
                )
                self.panels.append(newpanel)
                self.notebook_main.AddPage(newpanel, pagetitle)
        newpanel = ChoicePropertyPanel(
            self.notebook_main, id=wx.ID_ANY, context=self.context, choices="newly-speedchart"
        )
        self.panels.append(newpanel)
        self.notebook_main.AddPage(newpanel, _("Raster Chart"))

        newpanel = ChoicePropertyPanel(
            self.notebook_main, id=wx.ID_ANY, context=self.context, choices="newly-effects"
        )
        self.panels.append(newpanel)
        self.notebook_main.AddPage(newpanel, _("Effects"))

        newpanel = ChoicePropertyPanel(
            self.notebook_main, id=wx.ID_ANY, context=self.context, choices="newly-defaults"
        )
        self.panels.append(newpanel)
        self.notebook_main.AddPage(newpanel, _("Operation Defaults"))

        newpanel = WarningPanel(self.notebook_main, id=wx.ID_ANY, context=self.context)
        self.panels.append(newpanel)
        self.notebook_main.AddPage(newpanel, _("Warning"))

        newpanel = DefaultActionPanel(self.notebook_main, id=wx.ID_ANY, context=self.context)
        self.panels.append(newpanel)
        self.notebook_main.AddPage(newpanel, _("Default Actions"))

        newpanel = FormatterPanel(self.notebook_main, id=wx.ID_ANY, context=self.context)
        self.panels.append(newpanel)
