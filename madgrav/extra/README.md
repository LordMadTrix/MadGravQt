...
Many of these resemble addons that are built-in to madgrav.

## Architecture

The extra modules directory contains a collection of specialized plugins and utilities that extend MadGrav's functionality without being core dependencies. Each module follows the standard MadGrav plugin pattern with lifecycle hooks (`invalidate`, `register`, `boot`, etc.) and integrates through the kernel service system.
...
Automatic version checking and update notification system for MadGrav.
...
### Plugin Lifecycle
All extra modules follow MadGrav's plugin lifecycle:
...
This collection of extra modules significantly extends MadGrav's capabilities while maintaining the core system's modularity and optional nature.