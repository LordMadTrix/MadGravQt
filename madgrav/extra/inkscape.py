    ...
    if lifecycle == "register":
        ...
        def check_for_features(pathname, **kwargs):
            ...
            if conversion_preference == 1:
                # Ask
                msg = _(
                    "This file contains certain features that might not be fully supported by MadGrav"
                )
                for feat, entry in features.items():
                    if entry[0]:
                        msg += "\n" + f" - {feat}"
        ...
        stip = (
            _("Madgrav does not support all svg-features, so you might want")
            + "\n"
        ...
        choices = [
            {
                ...
                "label": _("Inkscape"),
                "tip": _(
                    "Path to inkscape-executable. Leave empty to let Madgrav establish standard locations"
                ),
                "page": "Input/Output",
