...
from madgrav.core.node.node import Fillrule, Linejoin
from madgrav.core.units import UNITS_PER_INCH, Length
from madgrav.core.geomstr import BeamTable, Geomstr
from madgrav.tools.jhfparser import JhfFont, SimplexFont, STD_FONT_FILE
from madgrav.tools.shxparser import ShxFont, ShxFontParseError
from madgrav.tools.ttfparser import TrueTypeFont, TTFParsingError
...
class MadgravFonts:
    ...
    def available_fonts(self):
            ...
            if not cache_valid:
                ...
                if not any(f[0] == STD_FONT_FILE for f in self._available_fonts):
                    self._available_fonts.insert(
                        0,
                        (STD_FONT_FILE, "MadGrav Simple", "Hershey", "Regular", False)
                    )
                # t1 = perf_counter()
        ...
        self._available_fonts.append( (STD_FONT_FILE, "MadGrav Simple", "Hershey", "Regular", False) )
        ...
        choices = [
            {
                ...
                "tip": _("Places where MadGrav will look for fonts."),
            },
        ]
        kernel.register_choices("preferences", choices)
        context.fonts = MadgravFonts(context=context)
