...
Provides and registers the GRBL driver for madgrav.

The GRBL module implements comprehensive support for GRBL (G-code based laser controller) devices, enabling MadGrav to communicate with a wide variety of CNC laser cutters and engravers. This module follows the standard MadGrav device driver pattern with device service, driver, and controller components for robust G-code generation and hardware control.
...
- **`driver.py`** - GRBLDriver implementation
  - Translates MadGrav cutcode operations to G-code commands
...
- **`plugin.py`** - Device registration and lifecycle management
  - Registers multiple GRBL device variants with MadGrav kernel
...
- **MadGrav Projects**: Native `.mk` files with GRBL export
...
This module provides MadGrav with comprehensive GRBL controller support, enabling reliable communication with a wide variety of G-code based laser systems.
