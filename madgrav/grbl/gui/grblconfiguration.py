...
from madgrav.gui.wxutils import ScrolledPanel, StaticBoxSizer
...
class GRBLConfiguration(MWindow):
    def __init__(self, *args, **kwds):
        ...
        panel_global = ChoicePropertyPanel(
            self.notebook_main, wx.ID_ANY, context=self.context, choices="grbl-advanced"
        )
        ...
        panel_protocol = ChoicePropertyPanel(
            self.notebook_main, wx.ID_ANY, context=self.context, choices="protocol"
        )

        panel_effects = ChoicePropertyPanel(
            self.notebook_main, wx.ID_ANY, context=self.context, choices="grbl-effects"
        )
        panel_defaults = ChoicePropertyPanel(
            self.notebook_main, wx.ID_ANY, context=self.context, choices="grbl-defaults"
        )
        panel_esp3d = ESP3DConfigPanel(
            self.notebook_main, wx.ID_ANY, context=self.context
        )
        panel_warn = WarningPanel(self.notebook_main, id=wx.ID_ANY, context=self.context)
        panel_actions = DefaultActionPanel(self.notebook_main, id=wx.ID_ANY, context=self.context)
        panel_formatter = FormatterPanel(self.notebook_main, id=wx.ID_ANY, context=self.context)
