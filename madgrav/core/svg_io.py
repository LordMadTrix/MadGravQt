...
from madgrav.core.node.node import Fillrule, Linecap, Linejoin
...
MADGRAV_NAMESPACE = "https://github.com/madgrav/madgrav/wiki/Namespace"
MADGRAV_XMLS_ID = "madgrav"
# Files saved before the meerk40t -> madgrav rebrand used this namespace;
# kept read-only so those older project files still load correctly.
LEGACY_MEERK40T_NAMESPACE = "https://github.com/meerk40t/meerk40t/wiki/Namespace"
...
class SVGProcessor:
    ...
    def _parse_path(
        ...
        There were a few versions of madgrav where Path was used to store other save nodes. But, there is not
    ...
    def _parse_element(self, element, ident, label, lock, context_node, e_list):
        ...
        madgrav contain notes and operations. Element type="elem point", and other points will also load with
        this code.
        ...
        if tag is not None:
            ...
            full_ns = f"{{{MADGRAV_NAMESPACE.lower()}}}"
            legacy_ns = f"{{{LEGACY_MEERK40T_NAMESPACE.lower()}}}"
            if full_ns in tag:
                tag = tag.replace(full_ns, "")
            elif legacy_ns in tag:
                tag = tag.replace(legacy_ns, "")
        ...
        if node_type == "op":
            # Madgrav 0.7.x fallback node types.
