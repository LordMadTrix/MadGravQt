"""
Path Offset Module for MadGrav
...
from madgrav.core.node.node import Linejoin
...
def path_offset(
    ...
    """
    try:
        from madgrav.svgelements import Path, Move, Line, Point, Close
    except Exception:
        return None
    ...
    def _poly_clockwise(points):
        # Handle both Point objects and tuples -- checked once per call, not
        # per point: hasattr() on every access showed up as the dominant
        # cost of offset/kerf computation under profiling (millions of
        # calls on documents with a few hundred shapes).
    ...
    def _poly_area(points):
        ...
        # Handle both Point objects and tuples -- checked once per call, see
        # _poly_clockwise above for why this isn't done per point.
