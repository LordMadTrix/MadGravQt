                    ...
                    for k in range(columns):
                        if j != (cy - 1) or k != (cx - 1):
                            ...
                            for e in add_elem:
                                ...
                                # Same fix as clipboard.py's "paste": e's
                                # shallow copy() can carry over a stale
                                # cached bounds from the source node.
            ...
            with self.node_lock:
                ...
                for cc in range(1, repeats):
                    ...
                    for e in add_elem:
                        ...
                        # Same fix as clipboard.py's "paste": invalidate
                        # whatever bounds cache e's shallow copy() carried
                        # over from the source node -- either branch above
                        # can leave it stale, so use the generic notifier
                        # rather than assuming which specific transform ran.
                        e.modified()
                        self.elem_branch.add_node(e)
