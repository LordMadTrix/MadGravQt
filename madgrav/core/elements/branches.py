...
from madgrav.core.elements.element_types import op_nodes
...
from madgrav.core.geomstr import NON_GEOMETRY_TYPES
        ...
        if data_type == "ops":
            ...
            for e, delta in zip(add_elem, shift):
                ...
                if tx != 0 or ty != 0:
                    ...
                    e.matrix *= matrix
                    # Same fix as clipboard.py's "paste": without this,
                    # a bounds cache carried over by the shallow copy()
                    # above (if the source node's bounds were ever read
                    # before this copy) never gets invalidated for the
                    # new, offset copy.
                    e.translated(tx, ty)
                with self.node_lock:
