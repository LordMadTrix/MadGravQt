...
def init_commands(kernel):
    ...
    def clipboard_paste(
        ...
        if dx != 0 or dy != 0:
            ...
            for node in pasted:
                node.matrix *= matrix
                # Matches the "translate" command's own matrix mutation
                # (madgrav/core/elements/shapes.py) -- without this, the
                # node's cached .bounds keeps whatever value it last had
                # (possibly copied verbatim from the source node by the
                # shallow copy() above, if something read its bounds
                # before this paste) forever, since nothing else here
                # invalidates it.
