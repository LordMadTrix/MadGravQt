...
from madgrav.core.node.node import Fillrule, Node
from madgrav.core.treeop import (
    get_tree_operation,
    tree_calc,
    tree_check,
    tree_conditional,
...
from madgrav.core.units import UNITS_PER_INCH, Length
from madgrav.kernel import CommandSyntaxError
from madgrav.svgelements import Matrix, Point
...
def init_tree(kernel):
    ...
    def blob2path(node, **kwargs):
        cancelled = False
        from madgrav.tools.driver_to_path import DriverToPath
    ...
    def egv2path(node, **kwargs):
        from madgrav.lihuiyu.parser import LihuiyuParser
        ...
        def copy_a_group(groupnode, parent, dx, dy):
            ...
            for orgnode in groupnode.children:
                ...
                if hasattr(copy_node, "matrix"):
                    ...
                    # Without this, copy_node's cached bounds can carry
                    # over verbatim from orgnode's shallow copy() above
                    # (if orgnode's bounds were ever read before this
                    # duplication) and never get invalidated for the new,
                    # offset copy -- same fix as clipboard.py's "paste".
                    copy_node.translated(dx, dy)
        ...
        def copy_single_node(orgnode, orgparent, times, dx, dy):
            ...
            for n in range(times):
                ...
                if hasattr(copy_node, "matrix"):
                    ...
                    # See copy_a_group's identical fix above.
                    copy_node.translated((n + 1) * dx, (n + 1) * dy)
                # Need to add stroke and fill, as copy will take the
                # default values for these attributes
                options = ["fill", "stroke", "wxfont"]
                for optional in options:
