...
from madgrav.core.node.node import Fillrule, Linecap, Linejoin, Node
from madgrav.core.units import (
    UNITS_PER_MM,
    UNITS_PER_PIXEL,
    UNITS_PER_POINT,
    Angle,
...
from madgrav.kernel import CommandSyntaxError
from madgrav.svgelements import (
    SVG_RULE_EVENODD,
    SVG_RULE_NONZERO,
    Color,
    Matrix,
...
from madgrav.core.geomstr import Geomstr, stitch_geometries, stitcheable_nodes
...
def init_commands(kernel):
        ...
        if prop is None or (prop == "?" and new_value == "?"):
            ...
            if prop == "?":
                ...
                for op in data:
                    ...
                    channel(prop_str)
                channel(
                    "Be careful what you do - this is a failsafe method to crash MadGrav, burn down your house or whatever..."
                )
            return
    ...
    def create_pattern(command, channel, _, data=None, post=None, **kwargs):
        ...
        from madgrav.tools.pathtools import VectorMontonizer
    ...
    def element_position(
        ...
        with self.undoscope("Position"):
            changes = False
            # union_bounds() defaults to ignore_locked=True, which is right
            # for e.g. "select all" bounding but wrong here: `data` IS the
            # set of nodes being positioned (each already permission-gated
            # below via can_move()), so excluding a locked-but-movable one
            # from its own reference bounds left dbounds at the sentinel
            # (inf, inf, -inf, -inf) whenever every selected node was
            # locked -- dx/dy came out +-inf and post_translate() wrote
            # that straight into the node's matrix, permanently wrecking
            # its geometry (confirmed by reproduction: position a locked
            # element -> its bounds become -inf).
            dbounds = Node.union_bounds(data, ignore_locked=False)
            for node in data:
                if not node.can_move(self.lock_allows_move):
                    continue
                nbounds = node.bounds
    ...
    def element_move_to_laser(command, channel, _, data=None, **kwargs):
        ...
        with self.undoscope("Translate to laser"):
            try:
                # See element_position: data IS the set of nodes being
                # moved (each permission-gated below via can_move()), so
                # ignore_locked=True would drop locked-but-movable nodes
                # from the reference bounds, leaving the sentinel
                # (inf, inf, -inf, -inf) whenever every selected node is
                # locked and corrupting geometry via post_translate(+-inf).
                bounds = Node.union_bounds(data, ignore_locked=False)
                otx = bounds[0]
                oty = bounds[1]
                ntx = tx - otx
                nty = ty - oty
