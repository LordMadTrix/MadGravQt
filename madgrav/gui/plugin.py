...
def plugin(kernel, lifecycle):
    ...
    if lifecycle == "invalidate":
        ...
        try:
            ...
            print("wxMadGrav plugin could not load because wxPython is not installed.")
            return True
...
and a wxpython version <= 4.1.1."""
    ...
    if lifecycle == "preregister":
        ...
        from madgrav.gui.fonts import wxfont_to_svg
        from madgrav.gui.laserrender import LaserRender
        from madgrav.gui.wxmadgrav import wxMadGrav

        kernel.register("module/wxMadGrav", wxMadGrav)
        kernel_root.open("module/wxMadGrav")
    ...
    if lifecycle == "register":
        # Safe defaults so any wx panel touching these (there are ~20)
        # doesn't AttributeError if it opens before "mainloop" -- e.g. a
        # window restored at kernel "poststart" (which runs before
        # "mainloop") or one opened on demand from the --qt shell. The real
        # measured values overwrite these once wx is ready to detect them.
        kernel_root.setting(float, "bitmap_correction_scale", 1.0)
        kernel_root.setting(bool, "faulty_bitmap_scaling", False)
        kernel_root.setting(float, "user_scale", 100)
        # context.gui is a live object reference (the main wx window, once
        # one exists), not a persistable preference, so it isn't a
        # .setting() -- just make sure the attribute exists.
        if not hasattr(kernel_root, "gui"):
            kernel_root.gui = None

        from madgrav.gui.themes import Themes

        kernel.add_service("themes", Themes(kernel))

        from madgrav.gui.guicolors import GuiColors

        kernel.add_service("colors", GuiColors(kernel))

        from madgrav.gui.scene.scene import Scene
        ...
        choices = [
            ...
            },
            {
                ...
                "tip": _("Should MadGrav remember its last position on the screen?"),
                "page": "Gui",
                # Hint for translation _("General")
                "section": "General",
            },
            ...
            },
            {
                ...
                "tip": "\n".join(
                    (
                        ...
                        _(
                            "Note: You will need to restart MadGrav for any change to take effect."
                        ),
                    )
                ),
                "page": "Gui",
            ...
            choices.append(
                {
                    ...
                    "tip": _("Don't show the warning when running MadGrav as root/admin on Linux"),
                    "page": "Gui",
                    # Hint for translation _("General")
                    "section": "General",
                }
        ...
        if kernel.is_shutdown or getattr(kernel.root, "_gui_mainloop_started", False):
            # This branch does one-time GUI bring-up (popup helpers,
            # BusyInfo, and -- under classic wx -- the blocking
            # madgravgui.MainLoop() call) and must never run twice, nor
            # start at all once a shutdown is already in progress:
            #  - kernel.is_shutdown: e.g. "-e quit" fires during poststart,
            #    which recursively drives the lifecycle all the way to
            #    shutdown before this branch would otherwise get its only
            #    chance to run -- reading a persistent setting here (DPI
            #    detection) then hits settings.py's post-termination guard
            #    and crashes.
            #  - _gui_mainloop_started: a kernel shutdown requested from a
            #    background thread while the app is already running (e.g.
            #    "quit" via a telnet consoleserver connection) re-enters
            #    the kernel lifecycle recursively and would otherwise call
            #    MainLoop() a second time from the wrong thread,
            #    deadlocking the whole process.
            return
        kernel.root._gui_mainloop_started = True
        ...
        from madgrav.gui.busy import BusyInfo
        ...
        def notification_message(message=None, **kwargs):
            ...
            msg = NotificationMessage(title="MadGrav", message=message)
            msg.Show()
        ...
        if kernel._hard_gui:
            # Icon/bitmap-scaling globals used by any wx panel, including
            # ones opened on demand from the --qt shell (which never runs
            # the kernel._gui block below, so never showed the classic main
            # window) -- without this, opening e.g. a device configuration
            # panel from Qt raised AttributeError on bitmap_correction_scale.

            def detect_windows_dpi(context):
                """
                Get the Windows DPI scaling factor for the current device.

                Process DPI awareness itself is set once, earlier and more
                precisely (Per-Monitor-v2 with a system-wide fallback), in
                wxMadGrav.__init__ -- calling SetProcessDpiAwareness again
                here would just fail (Windows only allows setting it once
                per process), so this only queries the current scale.
                """
                scale = 100
                if not (
                ...
                try:
                    ...
                    scale = OleDLL("shcore").GetScaleFactorForDevice(0)
                except (AttributeError, ImportError, OSError):
                    # This is the wrong windows version, or we get a
                    pass
        ...
        if kernel._gui:
            ...
            image = icons.icon_madgrav.GetBitmap(resize=sizeme)
            from ..main import APPLICATION_VERSION
            kernel.busyinfo.start(
                msg=_(
                    "Start MadGrav|V. {version}".format(
            ...
            madgravgui = kernel_root.open("module/wxMadGrav")
            ...
            kernel.console("window open MadGrav\n")
            ...
            try:
                ...
                print(f"MadGrav encountered an error at shutdown: {e}")
                pass
