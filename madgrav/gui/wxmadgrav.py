...
from madgrav.gui.consolepanel import Console
from madgrav.gui.navigationpanels import Navigation
from madgrav.gui.spoolerpanel import JobSpooler

# try:
#     # According to https://docs.wxpython.org/wx.richtext.1moduleindex.html
#     # richtext needs to be imported before wx.App i.e. wxMadGrav is instantiated
#     # so, we are doing it here even though we do not refer to it in this file
#     # richtext is used for the Console panel.
#     from wx import richtext
# except ImportError:
#     pass
from madgrav.gui.themes import Themes
from madgrav.gui.thread_info import ThreadInfo
from madgrav.gui.wxmscene import SceneWindow
from madgrav.gui.wxutils import TextCtrl, wxButton, wxStaticText
from madgrav.kernel import CommandSyntaxError, Module, get_safe_path
from madgrav.kernel.kernel import Job
...
from .wxmmain import MadGrav

"""
Laser software for the Stock-LIHUIYU laserboard.

MadGrav (pronounced MeerKat) is a built-from-the-ground-up MIT licensed
open-source laser cutting software. See https://github.com/madgrav/madgrav
for full details.

wxMadGrav is the primary gui addon for MadGrav. It requires wxPython for the interface.
...
class wxMadGrav(wx.App, Module):
    """
    wxMadGrav is the wx.App main class and a qualified Module for the MadGrav kernel.
    Running MadGrav without the wxMadGrav gui is both possible and reasonable. This should not change the way the
    ...
    def __init__(self, context, path):
        ...
        try:
            if hasattr(self, "SetAppearance") and hasattr(wx.App, "Appearance"):
                self.SetAppearance(wx.App.Appearance.Dark)
            if hasattr(self, "MSWEnableDarkMode") and hasattr(wx.App, "DarkMode_Always"):
                self.MSWEnableDarkMode(wx.App.DarkMode_Always)
        except Exception:
            pass
        ...
        if platform.system() == "Windows" and high_dpi:
            try:
                ...
                # Per-Monitor v2 (Windows 10 1703+): the modern, recommended
                # mode -- keeps rendering crisp when the window is dragged
                # between monitors with different scaling, and lets wx
                # auto-scale non-client areas. Falls back to the older
                # system-wide modes on Windows versions that don't support it.
                DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2 = -4
                if not windll.user32.SetProcessDpiAwarenessContext(
                    DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2
                ):
                    raise OSError("Per-Monitor v2 not supported")
            except (AttributeError, ImportError):
                # We're on a non-Windows box.
                pass
            except OSError:
                # Windows < 10 1703, or Per-Monitor v2 unsupported/denied --
                # fall back to the older system-wide DPI awareness API.
    ...
    def _get_usb_config_instructions(self):
        ...
        """
        instructions = (
            '# MadGrav USB Device Configuration Instructions' ,
            '# 1) Create file: /etc/udev/rules.d/99-madgrav.rules',
    ...
    def _show_sudo_warning(self):
        ...
        message = _(
            "WARNING: Running MadGrav as root/administrator!\n\n" +
            "Running MadGrav with elevated privileges (sudo/admin mode) may lead to unexpected behavior and is not recommended.\n\n" +
            "Potential issues:\n" +
        ...
        ) + "\n\n" + instructions +  "\n\n" + _("It is strongly recommended to run MadGrav as a regular user instead of using sudo.")
    ...
    def OnInit(self):
        self.name = f"MadGrav-{wx.GetUserId()}"
    ...
    def sub_register(kernel):
        ...
        def window_base(channel, _, path=None, remainder=None, **kwargs):
            """
            Opens a MadGrav window or provides information. This command is restricted to use with the wxMadGrav gui.
    ...
    def module_open(self, *args, **kwargs):
        ...
        from madgrav.gui.help_assets.help_assets import asset
        ...
        kernel.register("window/MadGrav", MadGrav)
        ...
        from madgrav.gui.helper import register_panel_helper

        kernel.register("wxpane/helper", register_panel_helper)

        from madgrav.gui.wxmribbon import register_panel_ribbon

        kernel.register("wxpane/Ribbon", register_panel_ribbon)

        from madgrav.gui.wxmscene import register_panel_scene

        kernel.register("wxpane/ScenePane", register_panel_scene)

        from madgrav.gui.wxmtree import register_panel_tree

        kernel.register("wxpane/Tree", register_panel_tree)

        from madgrav.gui.laserpanel import register_panel_laser

        kernel.register("wxpane/LaserPanel", register_panel_laser)

        from madgrav.gui.position import register_panel_position

        kernel.register("wxpane/Position", register_panel_position)

        from madgrav.gui.opassignment import register_panel_operation_assign

        kernel.register("wxpane/opassign", register_panel_operation_assign)

        from madgrav.gui.snapoptions import register_panel_snapoptions

        kernel.register("wxpane/Snap", register_panel_snapoptions)

        from madgrav.gui.magnetoptions import register_panel_magnetoptions

        kernel.register("wxpane/magnet", register_panel_magnetoptions)

        from madgrav.gui.wordlisteditor import register_panel_wordlist

        kernel.register("wxpane/wordlist", register_panel_wordlist)

        # from madgrav.gui.auitoolbars import register_toolbars
        ...
        from madgrav.gui.dialogoptions import DialogOptions
        ...
        if context.debug_mode:
            from madgrav.gui.mkdebug import (
            ...
            from madgrav.gui.utilitywidgets.debugwidgets import register_widget_icon
        ...
        choices = [
            {
                ...
                "tip": _("Allow only a single instance of MadGrav."),
                "page": "Start",
            },
            {
                "attr": "beep_soundfile",
                ...
                "label": _("Soundfile"),
                "tip": _(
                    "Define the soundfile MadGrav will play when the 'beep' command is issued"
                )
                + addon,
                "page": "Start",
            },
    ...
    def update_language(self, lang):
        ...
        if self.locale.IsOk() or platform.system() == "Linux":
            self.locale.AddCatalog("madgrav")
        else:
            self.locale = None
...
# end of class MadGravGui

MADGRAV_HOST = "dev.madgrav.com"
...
def send_data_to_developers(filename, data):
    ...
    host = MADGRAV_HOST  # Replace with the actual host
    port = 80  # Replace with the actual port

    # Construct the HTTP request
    boundary = "----------------madgrav-boundary"
    body = (
        f"--{boundary}\r\n"
...
def handleGUIException(exc_type, exc_value, exc_traceback):
    ...
    error_log = (
        f"MadGrav crash log. Version: {APPLICATION_VERSION} on {platform.system()}: "
    ...
    # Ask to send file.
    message = _(
        "The bad news is that MadGrav encountered a crash, and the developers apologise for this bug!\n\n" +
        "The good news is that you can help us fix this bug by anonymously sending us the crash details."
    )
    message += "\n" + _(
        "Only the crash details below are sent. No data from your MadGrav project is sent. No "
        + "personal information is sent either.\n"
        + "Send the following data to the MadGrav team?"
    )
