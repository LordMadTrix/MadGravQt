...
# from madgrav.core.exceptions import BadFileError
from madgrav.gui.gui_mixins import FormatPainter, Warnings
from madgrav.gui.statusbarwidgets.defaultoperations import DefaultOperationWidget
from madgrav.gui.statusbarwidgets.infowidget import (
    BurnProgressPanel,
    InformationWidget,
    StatusPanelWidget,
)
from madgrav.gui.statusbarwidgets.selectionwidget import (
    SelectionOptionWidget,
    SnapOptionsWidget,
)
from madgrav.gui.statusbarwidgets.shapepropwidget import (
    FillruleWidget,
    LinecapWidget,
    LinejoinWidget,
    PositionWidget,
)
from madgrav.gui.statusbarwidgets.statusbar import CustomStatusBar
from madgrav.gui.statusbarwidgets.strokewidget import ColorWidget, StrokeWidget
from madgrav.gui.wxutils import TextCtrl, wxButton, wxComboBox, wxStaticText
from madgrav.kernel import Job, get_safe_path, lookup_listener, signal_listener
from madgrav.main import APPLICATION_NAME
    ...
    def __init__(self, context, *args, **kwargs):
        ...
        choices = [
            ...
            },
            {
                ...
                "tip": _("How often should MadGrav save the current workspace"),
                "page": "Options",
                # Hint for translation _("Autosave")
                "section": "Autosave",
...
class MadGrav(MWindow):
    ...
    def __init__(self, *args, **kwds):
        ...
        import madgrav.gui.icons as icons
        ...
        art.SetColour(aui.AUI_DOCKART_ACTIVE_CAPTION_COLOUR, bg_col)
        art.SetColour(aui.AUI_DOCKART_ACTIVE_CAPTION_GRADIENT_COLOUR, bg_col)
        art.SetColour(aui.AUI_DOCKART_ACTIVE_CAPTION_TEXT_COLOUR, fg_col)

        bg_col = self.context.themes.get("inactive_bg")
        fg_col = self.context.themes.get("inactive_fg")
        art.SetColour(aui.AUI_DOCKART_INACTIVE_CAPTION_COLOUR, bg_col)
        art.SetColour(aui.AUI_DOCKART_INACTIVE_CAPTION_GRADIENT_COLOUR, bg_col)
        art.SetColour(aui.AUI_DOCKART_INACTIVE_CAPTION_TEXT_COLOUR, fg_col)
        art.SetColour(aui.AUI_DOCKART_BACKGROUND_COLOUR, self.context.themes.get("win_bg"))
        art.SetColour(aui.AUI_DOCKART_SASH_COLOUR, self.context.themes.get("win_bg"))
        ...
        if self.context.setting(bool, "pause_on_suspend", True):
            ...
            for device in available_devices:
                ...
                if not hasattr(device, "spooler") or device.spooler is None:
                    continue
                active = any(
                    hasattr(e, "is_running") and e.is_running()
                    for e in device.spooler.queue
                )
                if active:
    ...
    def register_options_and_choices(self, context):
        ...
        choices = [
            ...
            },
            {
                ...
                "tip": _(
                    "When you design an element, e.g. a line, then MadGrav will allow you to immediately create the next instance of this type."
                )
                + "\n"
                + _(
    ...
    def sub_register(kernel):
        ...
        def contains_a_param():
            from madgrav.core.elements.element_types import effect_nodes, elem_nodes
            ...
            services = [
                {
                    ...
                    "start_tip": _(
                        "Starts a webserver instance to control MadGrav from a webbrowser"
                    ),
                ...
                },
                {
                    ...
                    "start_tip": _(
                        "Starts a telnet instance to control MadGrav from a terminal"
                    ),
    ...
    def __set_help_menu(self):
        ...
        def launch_help_osx(event=None):
            _resource_path = "help/madgrav.help"
            if not os.path.exists(_resource_path):
                try:  # pyinstaller internal location
                    # pylint: disable=no-member
                    _resource_path = os.path.join(sys._MEIPASS, "help/madgrav.help")
                except AttributeError:
                    pass
            if not os.path.exists(_resource_path):
                try:  # Mac py2app resource
                    _resource_path = os.path.join(
                        os.environ["RESOURCEPATH"], "help/madgrav.help"
                    )
                except KeyError:
                    pass
        ...
        def online_help(event):
            ...
            url = f"https://github.com/madgrav/madgrav/wiki/Online-Help-{section}"
            import webbrowser
        ...
        if platform.system() == "Darwin":
            menuitem = self.help_menu.Append(
                wx.ID_HELP, _("&MadGrav Help"), _("Open the MadGrav Mac help file")
            )
            self.Bind(wx.EVT_MENU, launch_help_osx, id=menuitem.GetId())
            menuitem = self.help_menu.Append(
                wx.ID_ANY, _("&Online Help"), _("Open the Madgrav online wiki")
            )
            self.Bind(
                wx.EVT_MENU,
            ...
            menuitem = self.help_menu.Append(
                wx.ID_HELP,
                _("&Help\tF1"),
                _("Open the Madgrav online wiki Beginners page"),
            )
            self.Bind(
                wx.EVT_MENU,
                # lambda e: self.context("webhelp help\n"),
        ...
        menuitem = self.help_menu.Append(
            wx.ID_ANY, _("&Github"), _("Visit Madgrav's Github homepage")
        )
        self.Bind(
            wx.EVT_MENU,
        ...
        menuitem = self.help_menu.Append(
            wx.ID_ANY,
            _("&Releases"),
            _("Check for a new release on Madgrav's Github releases page"),
        )
        self.Bind(
            wx.EVT_MENU,
        ...
        menuitem = self.help_menu.Append(
            wx.ID_ANY,
            _("&Facebook"),
            _("Get help from the K40 Madgrav Facebook group"),
        )
        self.Bind(
            wx.EVT_MENU,
        ...
        menuitem = self.help_menu.Append(
            wx.ID_ANY,
            _("&Discord"),
            _("Chat with developers to get help on the Madgrav Discord server"),
        )
        self.Bind(
            wx.EVT_MENU,
        ...
        menuitem = self.help_menu.Append(
            wx.ID_ANY,
            _("&Makers Forum"),
            _("Get help from the Madgrav page on the Makers Forum"),
        )
        self.Bind(
            wx.EVT_MENU,
        ...
        menuitem = self.help_menu.Append(
            wx.ID_ANY,
            _("Check for Updates"),
            _("Check whether a newer version of Madgrav is available"),
        )
        ...
        menuitem = self.help_menu.Append(
            ...
            _("&About MadGrav"),
            _(
                "Toggle the About window acknowledging those who contributed to creating Madgrav"
            ),
        )
        self.Bind(
            wx.EVT_MENU,
    ...
    def add_language_menu(self):
        ...
        trans = tl.GetAvailableTranslations("madgrav")
    ...
    def __set_properties(self):
        # begin wxGlade: MadGrav.__set_properties
        self.__set_titlebar()
        _icon = wx.NullIcon
        ...
        if self.load(pathname, preferred_loader, execution=True, replace_ops=True):
            try:
                if self.context.uniform_svg and pathname.lower().endswith("svg"):
                    # or (len(elements) > 0 and "madgrav" in elements[0].values):
    ...
    def on_click_new(self, event=None):  # wxGlade: MadGrav.<event_handler>
        self.clear_project()

    def on_click_open(self, event=None):  # wxGlade: MadGrav.<event_handler>
        self.context(".dialog_load\n")
        self.context(".tool none\n")

    def on_click_import(self, event=None):  # wxGlade: MadGrav.<event_handler>
    ...
    def on_click_exit(self, event=None):  # wxGlade: MadGrav.<event_handler>
        try:
            self.Close()
        except RuntimeError:
            pass

    def on_click_zoom_out(self, event=None):  # wxGlade: MadGrav.<event_handler>
        """
        Zoomout button press
        """
        self.context(f"scene zoom {1.0 / 1.5}\n")

    def on_click_zoom_in(self, event=None):  # wxGlade: MadGrav.<event_handler>
        """
        Zoomin button press
        """
        self.context(f"scene zoom {1.5}\n")

    def on_click_zoom_selected(self, event=None):  # wxGlade: MadGrav.<event_handler>
        """
    ...
    def on_click_zoom_bed(self, event=None):  # wxGlade: MadGrav.<event_handler>
        """
    ...
    def check_for_crash(self):
        ...
        if os.path.exists(crash_indicator) and os.path.exists(recovery_file):
            ...
            message = (
                _(
                    "Apparently MadGrav did crash during the last session, we apologize for this invconvenience."
                )
                + "\n"
            )
            message += (
