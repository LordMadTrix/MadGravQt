...
from madgrav.gui.navigationpanels import Drag, Jog, JogDistancePanel, MovePanel
...
from madgrav.core.planner import STAGE_PLAN_BLOB
from madgrav.kernel import lookup_listener, signal_listener
...
def register_panel_laser(window, context):
    ...
    laser_panel = LaserPanel(notebook, wx.ID_ANY, context=context)
    plan_panel = JobPanel(notebook, wx.ID_ANY, context=context)

    optimize_panel = OptimizePanel(notebook, wx.ID_ANY, context=context)
    # jog_drag = wx.Panel(window, wx.ID_ANY)
    jog_drag = ScrolledPanel(notebook, wx.ID_ANY)
    ...
    jog_drag.Layout()
    move_panel = MovePanel(notebook, wx.ID_ANY, context=context)
...
class OptimizePanel(wx.Panel):
    ...
    def __init__(self, *args, context=None, **kwds):
        ...
        if hasattr(top, "add_module_delegate"):
            top.add_module_delegate(self.optimize_panel)
