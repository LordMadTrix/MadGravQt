...
from madgrav.gui.icons import STD_ICON_SIZE
from madgrav.kernel import Job
...
class DropDown:
    ...
    **End-User Perspective:**
    This panel provides user interface controls for dropdown functionality in MadGrav."""
...
class Button:
    ...
    **End-User Perspective:**
    This panel provides user interface controls for button functionality in MadGrav."""
    ...
    def set_aspect(
        ...
        **kwargs,
        ...
        # Well...
        from madgrav.gui.icons import PyEmbeddedImage, VectorIcon
...
class RibbonPanel:
    ...
    **End-User Perspective:**
    This panel provides user interface controls for ribbon functionality in MadGrav."""
...
class RibbonPage:
    ...
    **End-User Perspective:**
    This panel provides user interface controls for ribbonpage functionality in MadGrav."""
...
class RibbonBarPanel(wx.Control):
    ...
    **End-User Perspective:**
    This panel provides user interface controls for ribbonbar functionality in MadGrav."""
...
# Tiger note: This will be helpful when making ribbon DPI aware but Madgrav isn't ready for that quite yet.
...
class Art:
    ...
    **End-User Perspective:**
    This panel provides user interface controls for art functionality in MadGrav."""
    ...
    def establish_colors(self):
        ...
        if self.color_mode == COLOR_MODE_DARK:
            try:
                bg_theme = self.parent.context.themes.get("win_bg")
            except Exception:
                bg_theme = wx.Colour(30, 30, 46)
            if bg_theme is None:
                bg_theme = wx.Colour(30, 30, 46)
            self.ribbon_background = bg_theme
            self.button_face = bg_theme
            self.inactive_background = bg_theme
            self.text_color = wx.Colour(235, 235, 245)
            self.text_color_inactive = wx.Colour(160, 160, 175)
            self.text_color_disabled = wx.Colour(100, 100, 115)
            self.black_color = wx.Colour(235, 235, 245)
            self.button_face_hover = wx.Colour(10, 132, 255)
    ...
    def _paint_tab(self, dc: wx.DC, page: RibbonPage):
        ...
        if page is not self.current_page:
            ...
            dc.SetBrush(wx.Brush(wx.Colour(45, 45, 65)))
    ...
    def _render_button_content(
        ...
        """
        bg = self.ribbon_background
        pen = wx.TRANSPARENT_PEN
        text_fg = self.text_color

        if not button.enabled:
            bg = self.ribbon_background
            text_fg = self.text_color_disabled
        elif button.toggle:
            bg = self.highlight
            text_fg = wx.Colour(255, 255, 255)
        elif self.hover_button is button and self.hover_dropdown is None:
            bg = self.button_face_hover
            text_fg = wx.Colour(255, 255, 255)
