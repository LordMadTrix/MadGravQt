...
class MWindow(wx.Frame, Module):
    """
    Madgrav window class does some of the more fancy operations for the madgrav windows. This includes saving and
    ...
    MadGrav Windows have a hook module_close and module_open into window_open and window_close to register and
    ...
    def __init__(self, width, height, context, path, parent, *args, style=-1, **kwds):
        if style == -1:
            ...
            # wx asserts (crashes) on FRAME_FLOAT_ON_PARENT with no parent to
            # float on -- some MWindow subclasses (Preferences, About,
            # Alignment, ImageSplitter) hardcode it in their own style=, which
            # normally works fine since the classic wx UI always has a main
            # window to parent against, but not when opened from --qt (no wx
            # main frame is ever shown there).
