"""madgrav.gui.materialmanager
--------------------------------

GUI for the Material Manager panel used by MadGrav.
...
from madgrav.core.elements.element_types import op_vector_nodes
...
from madgrav.gui.gui_optypes import get_operation_info
...
from madgrav.kernel.settings import Settings
...
class ImportDialog(wx.Dialog):
    ...
    def on_file(self, event):
        mydlg = wx.FileDialog(
            ...
            wildcard="Supported files|*.lib;*.ini;*.clb;*.cfg|EZcad files (*.lib;*.ini)|*.lib;*.ini|Lightburn files (*.clb)|*.clb|MadGrav operations (*.cfg)|*.cfg|All files (*.*)|*.*",
...
class MaterialPanel(ScrolledPanel):
    ...
    def __init__(self, *args, context=None, **kwds):
        ...
        self.btn_share.SetToolTip(
            _("Share the current library entry with the MadGrav community")
        )
    ...
    def on_share(self, event):
        ...
        dlg = wx.TextEntryDialog(
            self,
            _(
                "Thank you for your willingness to share your material setting with the MadGrav community.\n"
    ...
    def on_import(self, event, filename=None):
        ...
        if myfile.endswith(".clb"):
            ...
            added = self.import_madgrav(info)
        else:
    ...
    def on_preview_rightclick(self, event):
        ...
        def on_menu_popup_missing(*args):
            ...
            # "meerk40t:" is the legacy prefix from before the madgrav
            # rebrand -- kept read-only so older saved presets still work.
            mkpatterns = ("madgrav:", "meerk40t:")
            uid = list()
            for op in op_list:
                ...
                if not op.id:
                    continue
                if replace_mk_pattern and op.id.startswith(mkpatterns):
                    continue
                uid.append(op.id)
            for op in op_list:
                ...
                if not hasattr(op, "id"):
                    ...
                    # oldid = op.id
                    if op.id and not (
                        replace_mk_pattern and op.id.startswith(mkpatterns)
                    ):
                        replace_id = False
...
class MaterialManager(MWindow):
    def __init__(self, *args, **kwds):
        ...
        self.panel_library = MaterialPanel(self.notebook_main, wx.ID_ANY, context=self.context)
        # self.panel_import = ImportPanel(self.notebook_main, wx.ID_ANY, context=self.context)
