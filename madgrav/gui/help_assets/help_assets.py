...
You can share such a material setting with the MadGrav community and you can benefit from the contributions of others by loading and using their settings.
"""
...
Diese Daten können im Übrigen mit der Madgrav Commnity geteilt werden, un man im Gegenzug von den Beiträgen anderer profitieren.
"""
...
È possibile condividere queste impostazioni di lavorazione con la comunità di MadGrav e, allo stesso tempo, beneficiare dei contributi degli altri utenti scaricando e utilizzando le loro configurazioni
"""
...
Vous pouvez partager de tels paramètres de matériau avec la communauté MadGrav et vous pouvez bénéficier des contributions des autres en chargeant et en utilisant leurs paramètres.
"""
...
Вы можете поделиться такими настройками материала с сообществом MadGrav и можете извлечь пользу из вкладов других, загружая и используя их настройки.
"""
...
Puede compartir dicha configuración de material con la comunidad de MadGrav y beneficiarse de las contribuciones de otros cargando y utilizando sus configuraciones.
"""
chinese_material_howto = """
材料库管理器允许您创建、维护、使用和管理针对特定材料定制的操作（因此称为材料库）。
例如，用于切割亚克力的参数与用于在石板上雕刻图像的参数非常不同。
您可以与MadGrav社区分享此类材料设置，也可以通过加载和使用他人的设置来受益于他人的贡献。
"""
...
Az ilyen anyagbeállításokat megoszthatja a MadGrav közösséggel, és mások hozzájárulásaiból is profitálhat, ha betölti és használja az ő beállításaikat.
"""
...
Você pode compartilhar essa configuração de material com a comunidade MadGrav e pode se beneficiar das contribuições de outros carregando e usando suas configurações.
"""
...
このようなマテリアル設定をMadGravコミュニティと共有したり、他の人の設定を読み込んで利用することで、その貢献から恩恵を受けることができます。
"""
...
U kunt dergelijke materiaalsinstellingen delen met de MadGrav-gemeenschap en profiteren van de bijdragen van anderen door hun instellingen te laden en te gebruiken.
"""
...
Bu tür bir malzeme ayarını MadGrav topluluğu ile paylaşabilir ve başkalarının katkılarından yararlanmak için onların ayarlarını yükleyip kullanabilirsiniz.
"""
...
Możesz udostępnić takie ustawienie materiału społeczności MadGrav i możesz skorzystać z wkładów innych, ładując i używając ich ustawień.
"""
