...
def add_micro_tabs_to_path(path, tab_count=4, tab_size_mm=0.5):
    """
...
def apply_tabs_to_selected_nodes(elements_service, tab_count=4, tab_size_mm=0.5):
    """
    ...
    for node in nodes:
        if hasattr(node, "path") and node.path is not None:
            node.path = add_micro_tabs_to_path(node.path, tab_count=tab_count, tab_size_mm=tab_size_mm)
            node.altered()
            updated_count += 1
