...
The DXF (Drawing Exchange Format) module provides comprehensive support for importing AutoCAD DXF files into MadGrav. This module enables laser cutting software to work with industry-standard CAD file formats, converting DXF geometric entities into MadGrav's element system for laser processing.

## Architecture

The DXF module follows MadGrav's plugin architecture pattern with a clean separation of concerns:
...
- **DXFProcessor**: Core processing engine that converts DXF entities to MadGrav elements
...
- **Coordinate System**: Proper transformation from DXF coordinate system (+Y up) to MadGrav (+Y down)
...
- **Color Mapping**: DXF color conversion to MadGrav stroke colors
...
### Basic Import
```python
# DXF files are automatically loaded through MadGrav's file loading system
# Simply open a .dxf file in MadGrav
```

### Console Commands
The DXF module integrates with MadGrav's console system but doesn't provide additional commands beyond standard file loading.

### Programmatic Usage
```python
from madgrav.dxf.dxf_io import DxfLoader
...
DXF uses a bottom-left origin with +Y upward, while MadGrav uses top-left with +Y downward. The module applies:

```python
# Scale and flip Y-axis, then translate to MadGrav coordinate system
...
- **Wrong Scale**: Verify units in CAD software match MadGrav expectations
...
## Integration with MadGrav Core

The DXF module integrates deeply with MadGrav's core systems:
