...
class ConfigurationInterfacePanel(ScrolledPanel):
    def __init__(self, *args, context=None, **kwds):
        ...
        self.radio_tcp.SetToolTip(
            _(
                "Select this to connect this instance of Madgrav to another instance of Madgrav running as a remote server."
            )
        )
...
class LihuiyuDriverGui(MWindow):
    def __init__(self, *args, **kwds):
        ...
        panel_effects = ChoicePropertyPanel(
            self.notebook_main, wx.ID_ANY, context=self.context, choices="lhy-effects"
        )
        panel_defaults = ChoicePropertyPanel(
            self.notebook_main, wx.ID_ANY, context=self.context, choices="lhy-defaults"
        )
        panel_warn = WarningPanel(self.notebook_main, id=wx.ID_ANY, context=self.context)
        panel_actions = DefaultActionPanel(
            self.notebook_main, id=wx.ID_ANY, context=self.context
        )
        panel_format = FormatterPanel(self.notebook_main, id=wx.ID_ANY, context=self.context)
