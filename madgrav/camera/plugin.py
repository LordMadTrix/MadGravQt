...
def plugin(kernel, lifecycle=None):
    ...
    if lifecycle == "invalidate":
        ...
        @kernel.console_argument("subcommand", type=str, help="reset")
        @kernel.console_command(
            "align",
            help="align (reset) — clear the precise camera<->bed alignment homography",
            input_type="camera",
            output_type="camera",
        )
        def align_camera(data=None, subcommand=None, **kwargs):
            if subcommand is None:
                raise CommandSyntaxError
            if subcommand == "reset":
                data.reset_alignment_homography()
            else:
                raise CommandSyntaxError
            return "camera", data
