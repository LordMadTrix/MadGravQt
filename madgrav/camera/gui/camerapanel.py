...
from madgrav.camera.camera import FISHEYE_POSITIONS
from madgrav.gui.icons import (
...
from madgrav.gui.wxutils import (
    TextCtrl,
    wxBitmapButton,
    wxButton,
    wxCheckBox,
...
from madgrav.kernel import Job, signal_listener
    ...
    **End-User Perspective:**
    ...
    def __init__(
        ...
        if not pane:
            ...
            self.button_fisheye_undo = wxButton(self, wx.ID_ANY, _("Undo shot"))
        ...
        if not pane:
            ...
            self.button_detect.SetToolTip(
                _(
                    "Capture this lens calibration shot.\n"
                    "You need to print a 9x6 checkerboard pattern from OpenCV.\n"
                    "It should be flat and visible in the camera.\n"
                    "Follow the guided position shown below the toolbar,\n"
                    "moving the checkerboard between captures for best coverage."
                )
            )
            self.button_detect.SetSize(self.button_detect.GetBestSize())
            self.button_fisheye_undo.SetToolTip(
                _("Discard the last lens calibration shot and go back one step.")
            )
            ...
            sizer_controls.Add(self.button_fisheye_undo, 0, 0, 0)
            sizer_main.Add(sizer_controls, 1, wx.EXPAND, 0)
            self.label_fisheye_status = wxStaticText(self, wx.ID_ANY, "")
            sizer_main.Add(self.label_fisheye_status, 0, wx.EXPAND, 0)
            ...
            self.Bind(
                wx.EVT_BUTTON, self.undo_fisheye_shot, self.button_fisheye_undo
            )
        ...
        if not pane:
            ...
            self._update_fisheye_status_label()
    ...
    def update_camera_frame(self, event=None, force=False):
        ...
        from madgrav.camera.camera import make_bed_bitmap_from_frame
    ...
    def undo_fisheye_shot(self, event=None):
        """
        Discard the last lens calibration shot and go back one guided step.

        @param event:
        @return:
        """
        self.camera(f"camera{self.index} fisheye back\n")
        self._update_fisheye_status_label()
        self.widget_scene.request_refresh()

    def _update_fisheye_status_label(self):
        """Refresh the guided-shot instruction + colored RMS score label."""
        if not hasattr(self, "label_fisheye_status"):
            return
        shot_index = self.camera.fisheye_shot_index
        parts = []
        if shot_index < len(FISHEYE_POSITIONS):
            parts.append(
                _("Shot {n}/{total}: move checkerboard to '{pos}', then Capture.").format(
                    n=shot_index + 1,
                    total=len(FISHEYE_POSITIONS),
                    pos=_(FISHEYE_POSITIONS[shot_index]),
                )
            )
        else:
            parts.append(
                _("All {total} guided shots captured — you can keep going or stop here.").format(
                    total=len(FISHEYE_POSITIONS)
                )
            )
        rms = self.camera.fisheye_rms
        if rms is None:
            colour = wx.BLACK
        else:
            parts.append(_("Score: {rms:.3f}").format(rms=rms))
            if rms < 0.3:
        ...
        self.label_fisheye_status.SetForegroundColour(colour)
        self.label_fisheye_status.SetLabel("   ".join(parts))
        self.label_fisheye_status.Wrap(-1)
    ...
    def on_button_update(self, event=None):  # wxGlade: CameraInterface.<event_handler>
        ...
        @return:
        """
        from madgrav.camera.camera import camera_user_log
...
class CamInterfaceWidget(Widget):
    ...
    def event(self, window_pos=None, space_pos=None, event_type=None, **kwargs):
        if event_type == "rightdown":
            ...
            item = menu.Append(wx.ID_ANY, _("Undo Last Fisheye Shot"), "")
            self.cam.Bind(
                wx.EVT_MENU,
                self.cam.undo_fisheye_shot,
                id=item.GetId(),
            )
            menu.AppendSeparator()
...
class CameraInterface(MWindow):
    ...
    def create_menu(self, append):
        ...
        item = wxglade_tmp_menu.Append(wx.ID_ANY, _("Undo Last Fisheye Shot"), "")
        self.Bind(wx.EVT_MENU, self.panel.undo_fisheye_shot, id=item.GetId())
...
class CameraURIPanel(wx.Panel):
    ...
    def on_tree_popup_edit(self, index):
        def edit(event=None):
            ...
            if dlg.ShowModal() == wx.ID_OK:
                from madgrav.camera.camera import normalize_camera_uri
    ...
    def on_button_connect(self, event=None):
        from madgrav.camera.camera import connect_camera_from_paste
