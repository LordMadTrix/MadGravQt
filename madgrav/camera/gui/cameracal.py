...
from madgrav.gui.wxutils import StaticBoxSizer, wxButton, wxStaticText, TextCtrl
...
class CameraCalibWindow(MWindow):
    ...
    def __init__(self, *args, **kwds):
        ...
        precise_box = StaticBoxSizer(
            self, wx.ID_ANY, _("Precise alignment (burns a target)"), wx.VERTICAL
        )
        precise_box.Add(
            wxStaticText(
                self,
                wx.ID_ANY,
                _(
                    "Burns a numbered target on scrap material, then computes\n"
                    "the camera-to-bed mapping from a photo of it (like LightBurn),\n"
                    "with a real mm accuracy score. More accurate than the quick\n"
                    "eyeball steps below, but needs scrap material and a burn."
                ),
            ),
            0,
            wx.EXPAND | wx.BOTTOM,
        ...
        self.btn_open_align_wizard = wxButton(
            self, wx.ID_ANY, _("Open precise alignment wizard")
        )
        precise_box.Add(self.btn_open_align_wizard, 0, wx.EXPAND, 0)

        steps = StaticBoxSizer(self, wx.ID_ANY, _("Quick alignment (by eye)"), wx.VERTICAL)
        ...
        self.sizer.Add(precise_box, 0, wx.EXPAND | wx.ALL, 8)
        self.sizer.Add(steps, 0, wx.EXPAND | wx.LEFT | wx.RIGHT, 8)
        ...
        self.btn_open_align_wizard.Bind(wx.EVT_BUTTON, self.on_open_align_wizard)
    ...
    def _load_saved_camera_uri(self):
        from madgrav.camera.camera import _camera_service_for_index
    ...
    def on_open_align_wizard(self, event=None):
        self.context("window toggle CameraAlignWizard\n")
