...
# Guided lens-calibration shot sequence (mirrors LightBurn's "cover the
# whole frame" guidance: center, four corners, four edge midpoints).
FISHEYE_POSITIONS = [
    "Center",
    "Top-left corner",
    "Top-right corner",
...
# Internal resolution (pixels per mm) used for the bed-mm-proportioned
# alignment overlay produced by apply_alignment_homography().
ALIGNMENT_PX_PER_MM = 4


def compute_homography_and_error(pixel_pts, dest_pts):
    """
    Compute a homography mapping pixel_pts -> dest_pts (both Nx2, N>=4),
    using RANSAC when there are more than the minimal 4 correspondences,
    and the RMS reprojection error of that homography against the same
    points it was fit from (in dest_pts units).

    @param pixel_pts: sequence of N (x, y) source coordinates
    @param dest_pts: sequence of N (x, y) target coordinates, same order
    @return: (3x3 numpy homography matrix, rms error)
    """
    src = np.asarray(pixel_pts, dtype=np.float64).reshape(-1, 1, 2)
    dst = np.asarray(dest_pts, dtype=np.float64).reshape(-1, 1, 2)
    if len(src) < 4 or len(src) != len(dst):
        raise ValueError(
            "At least 4 point correspondences are required, in equal number."
        )
...
class Camera(Service):
    def __init__(self, kernel, camera_path, *args, **kwargs):
        ...
        self.fisheye_k = None
        self.fisheye_d = None
        # Transient (not persisted): last calibration RMS score and how far
        # through the guided FISHEYE_POSITIONS shot sequence we are.
        self.fisheye_rms = None
        self.fisheye_shot_index = 0
        if self.fisheye is not None and len(self.fisheye) != 0:
            self.fisheye_k, self.fisheye_d = self.fisheye

        # Alignment homography (camera pixel -> bed mm), set by the precise
        # alignment wizard. None means "use the legacy corner-drag +
        # perspective-stretch path" (see process_frame/background).
    ...
    def fisheye_capture(self):
        ...
        try:
            ...
            return
        self.fisheye_rms = float(rms)
        self.fisheye_shot_index = min(
            self.fisheye_shot_index + 1, len(FISHEYE_POSITIONS)
        )
        self.signal(
            "warning",
            _("Success. {count} images so far. Score: {rms:.3f}").format(
                count=len(self._object_points), rms=self.fisheye_rms
            ),
            _("Image Captured"),
            4 | 2048,
        )
    ...
    def get_alignment_capture_frame(self):
        """
        Return the current camera frame, oriented + fisheye-undistorted
        exactly like process_frame()'s pixel space — this is the pixel
        space that compute_alignment_homography()/apply_alignment_homography()
        expect clicked target coordinates to be in.

        @return: BGR numpy frame, or None if no frame is available yet.
        """
        frame = self._orient_frame(self._current_raw)
        if frame is None:
            frame = self._orient_frame(self._last_raw)
    ...
    def process_frame(self):
        ...
        if self.alignment_homography is not None:
            # Precise bed-mm homography from the alignment wizard supersedes
            # the legacy corner-drag perspective stretch below.
            warped = self.apply_alignment_homography(frame)
            if warped is not None:
                frame = warped
        elif self.correction_perspective:
    ...
    def backtrack_fisheye(self):
        if self._object_points:
            ...
            self.fisheye_shot_index = max(0, self.fisheye_shot_index - 1)
            # The displayed score no longer matches the current point set
            # until the next successful capture recomputes it.
            self.fisheye_rms = None
    ...
    def compute_alignment_homography(self, pixel_pts, bed_mm_pts):
        """
        Compute and store the camera-pixel -> bed alignment homography from
        N (4-9) clicked-target correspondences, and return the residual
        alignment error in mm.

        @param pixel_pts: sequence of N (px, py) clicked raw camera-image
            pixel coordinates, in click order.
        @param bed_mm_pts: sequence of N (mm_x, mm_y) known real bed
            positions of the same targets, same order as pixel_pts.
        @return: residual RMS error in mm (float)
        """
        dest_px_pts = [
            (mx * ALIGNMENT_PX_PER_MM, my * ALIGNMENT_PX_PER_MM)
            for mx, my in bed_mm_pts
        ]
        H, rms_px = compute_homography_and_error(pixel_pts, dest_px_pts)
        self.alignment_homography = H.tolist()
        rms_mm = rms_px / ALIGNMENT_PX_PER_MM
        self.alignment_rms = rms_mm
        return rms_mm

    def reset_alignment_homography(self):
        """
        Clear the precise alignment homography and fall back to the legacy
        corner-drag perspective stretch + mm nudge.

        @return:
        """
        self.alignment_homography = None
        self.alignment_rms = None

    def apply_alignment_homography(self, frame):
        """
        Warp a raw camera frame directly into bed-mm-proportioned pixel
        space using the stored alignment homography.

        @param frame: raw (post fisheye-correction) camera frame
        @return: warped frame sized to the bed's aspect ratio at
            ALIGNMENT_PX_PER_MM resolution, or None if no homography is
            stored or the device/view isn't available yet (caller should
            fall back to the legacy perspective path in that case).
        """
        if self.alignment_homography is None or frame is None:
            return None
        try:
            from madgrav.core.units import UNITS_PER_MM

            bed_width_mm = self.device.view.unit_width / UNITS_PER_MM
            bed_height_mm = self.device.view.unit_height / UNITS_PER_MM
        except AttributeError:
            return None
        dest_w = max(1, int(round(bed_width_mm * ALIGNMENT_PX_PER_MM)))
...
def composite_bed_photo_on_device_dc(scene, dc):
    ...
    from madgrav.gui.scenewidgets.bedwidget import _bed_rect
...
def get_main_scene_panel(kernel):
    """Return the docked main MadGravScenePanel, if the GUI is up."""
...
def iter_bed_widgets(widget):
    ...
    from madgrav.gui.scenewidgets.bedwidget import BedWidget
...
def _ensure_show_bed_background(context):
    from madgrav.gui.laserrender import DRAW_MODE_BACKGROUND
