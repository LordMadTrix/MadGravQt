...
class WebRequestHandler(BaseHTTPRequestHandler):
    ...
    def send_response_headers(
        ...
        self.send_header("Server", "MadGrav-WebServer")
        self.send_header(
...
class WebServer(Module):
    ...
    def __init__(
        ...
        # Per-instance CSRF token: required on every command-executing POST so
        # a page from another origin (or another local process) can't drive
        # the console -- it can't read this page to learn the token.
    ...
    def _get_html_scripts(self) -> str:
        ...
        # NOTE: token is prepended via concatenation, not an f-string --
        # the JS below is full of literal { } that an f-string would try
        # (and fail) to evaluate as Python placeholders.
        token_line = f'const CSRF_TOKEN = "{self.csrf_token}";\n'
        return token_line + """
        ...
        function sendJobCommand(jobId, operation) {
            fetch(window.location.href, {
                ...
                body: `job_cmd=${encodeURIComponent(jobId + ':' + operation)}&csrf_token=${encodeURIComponent(CSRF_TOKEN)}`
            })
            .then(() => {
        ...
        function sendDeviceCommand(command) {
            fetch(window.location.href, {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                body: `cmd=${encodeURIComponent(command)}&csrf_token=${encodeURIComponent(CSRF_TOKEN)}`
            })
            .then(() => {
    ...
    <div class="container">
        ...
        <div class="footer">
            Powered by MadGrav &copy; 2025 | Server running on port {self.port}
        </div>
    </div>
        """
