def plugin(kernel, lifecycle=None):
    if lifecycle == "register":
        ...
        @kernel.console_option(
            ...
            default="127.0.0.1",
            help=_(
                "address to listen on. Defaults to localhost-only: this telnet "
                "console executes any command it receives, unauthenticated, so "
                "only widen it (e.g. to 0.0.0.0) on a network you trust."
            ),
        )
        @kernel.console_option(
            "silent",
        ...
        def server_console(
            ...
            **kwargs,
            ...
            try:
                ...
                server = root.open_as(
                    "module/TCPServer", "console-server", port=port, address=bind
                )
                if quit:
