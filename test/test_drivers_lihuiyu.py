...
File version: 1.0.01
Copyright: Unknown
Creator-Software: MadGrav v0.0.0-testing
...
class TestDriverLihuiyu(unittest.TestCase):
    def test_reload_devices_lihuiyu(self):
        ...
        @return:
        """
        kernel = bootstrap.bootstrap(profile="MadGrav_LHY")
        try:
            for i in range(10):
        ...
        kernel = bootstrap.bootstrap(profile="MadGrav_LHY", ignore_settings=False)
        try:
...
class TestDriverLihuiyuRotary(unittest.TestCase):
    def test_driver_rotary_engrave(self):
        ...
        kernel = bootstrap.bootstrap(profile="MadGrav_TEST_rotary")
        try:
            kernel.console("service device start -i lhystudios 0\n")
