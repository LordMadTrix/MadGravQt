...
class TestDriverGalvo(unittest.TestCase):
    def test_reload_devices_galvo(self):
        ...
        @return:
        """
        kernel = bootstrap.bootstrap(profile="MadGrav_GALVO")
        try:
            for i in range(10):
        ...
        kernel = bootstrap.bootstrap(profile="MadGrav_GALVO", ignore_settings=False)
        try:
...
class TestDriverGalvoRotary(unittest.TestCase):
    def test_driver_rotary_engrave(self):
        ...
        kernel = bootstrap.bootstrap(profile="MadGrav_GALVO_rotary")
        try:
            kernel.console("service device start -i balor 0\n")
