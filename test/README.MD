...
This directory contains comprehensive automated tests for MadGrav's various functionalities to prevent regressions after code changes and ensure system stability.

## Test Frameworks

MadGrav supports multiple testing frameworks for flexibility and comprehensive coverage.
...
pytest --cov=madgrav --cov-report=html
```
...
All tests use a standardized bootstrap system (`bootstrap.py`) that creates a minimal MadGrav kernel with essential plugins for testing.
...
def test_console_command(self):
    ...
    try:
        ...
        # Verify output
        self.assertTrue(any("MadGrav" in line for line in output))
    finally:
        kernel()
```
...
def test_with_mock_device(self):
    ...
    with patch('madgrav.device.SomeDevice') as mock_device:
...
- Isolated test profile (`MadGrav_TEST`)
...
This comprehensive test suite ensures MadGrav maintains high quality and reliability across all supported platforms and use cases.