...
class TestDriverMoshi(unittest.TestCase):
    def test_reload_devices_moshi(self):
        ...
        @return:
        """
        kernel = bootstrap.bootstrap(profile="MadGrav_MOSHI")
        try:
            for i in range(10):
        ...
        kernel = bootstrap.bootstrap(profile="MadGrav_MOSHI", ignore_settings=False)
        try:
...
class TestDriverMoshiRotary(unittest.TestCase):
    def test_driver_rotary_engrave(self):
        ...
        kernel = bootstrap.bootstrap(profile="MadGrav_TEST_MOSHI_rotary")
        try:
            kernel.console("service device start -i moshi 0\n")
